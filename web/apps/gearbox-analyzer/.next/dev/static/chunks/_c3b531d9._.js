...
class VehicleComponent extends __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$drivetrain$2d$sim$2f$src$2f$core$2f$component$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["DrivetrainComponent"] {
   ...
   */ calcTotalRoadLoad(velocity, grade) {
        ...
        // Direction-aware (drag always opposes motion)
        const signV = velocity >= 0 ? 1 : -1;
        return fGrade + signV * (fRoll + fAero);
    }
    /**
...
class DrivetrainSimulator {
   ...
   */ simulate(x0, controller, gradeProfile = 0.0, config = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$drivetrain$2d$sim$2f$src$2f$simulation$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["DEFAULT_CONFIG"]) {
        ...
        // Run ODE integration
        const sol = (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$drivetrain$2d$sim$2f$src$2f$simulation$2f$integrator$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["integrateFixed"])(dynamicsWrapper, tEval, x0Arr, 'RK4');
        if (!sol.success) {
   ...
   */ async simulateWithProgress(x0, controller, gradeProfile = 0.0, config = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$drivetrain$2d$sim$2f$src$2f$simulation$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["DEFAULT_CONFIG"], onProgress) {
        ...
        for(let k = 1; k < tEval.length; k++){
            const t = tEval[k - 1];
            const dt = tEval[k] - t;
            // Compute dynamics at current state
            const state = this.drivetrain.arrayToState(x);
            const grade = gradeFn(t);
            const control = controlFn(t, state, grade);
            this._controlLog.push([
            ...
            const dx = this.drivetrain.dynamics(t, x, control, {
                grade
            });
            nfev++;
            // RK4 step
            const k1 = dx;
            const k2 = this.drivetrain.dynamics(t + dt / 2, x.map((xi, i)=>xi + dt / 2 * k1[i]), control, {
                grade
            });
            nfev++;
            const k3 = this.drivetrain.dynamics(t + dt / 2, x.map((xi, i)=>xi + dt / 2 * k2[i]), control, {
                grade
            });
            nfev++;
            const k4 = this.drivetrain.dynamics(t + dt, x.map((xi, i)=>xi + dt * k3[i]), control, {
                grade
            });
            nfev++;
 ...
 *
 * Shows tractive force at wheels vs vehicle speed for each gear.
 * Accounts for:
 * - Constant torque region (low speed, T = T_peak)
 * - Constant power region (high speed, T = P_rated / ω)
 */ function computeRimpull(nodes, edges) {
    ...
    const vehicleNode = nodes.find((n)=>n.data.componentType === "vehicle");
    if (!engineNode || !vehicleNode) {
        return curves;
    }
    const engineParams = engineNode.data.params;
    const vehicleParams = vehicleNode.data.params;
    // Engine parameters
    const tPeak = engineParams.tPeak || 11220; // N·m
    const pRated = engineParams.pRated || 1801000; // W
    const rpmIdle = engineParams.rpmIdle || 700;
    const rpmMax = engineParams.rpmMax || 1800;
    // Calculate base speed where power limiting begins
    // At base speed: P_rated = T_peak × ω_base
    const omegaBase = pRated / tPeak; // rad/s
    const rpmBase = omegaBase * 30 / Math.PI;
    // Vehicle parameters
    const rWheel = vehicleParams.rWheel || 1.78;
    const finalDrive = 16.0; // Final drive ratio
    const efficiency = 0.92; // Drivetrain efficiency
    // Get gear ratios
    ...
    for(let g = 0; g < gearRatios.length; g++){
        ...
        for(let i = 0; i <= numPoints; i++){
            ...
            // Tractive force at wheel: F = T × ratio × efficiency / r_wheel
    ...
    // Grade resistance for different grades
    const grades = [
        0,
        0.05,
        0.10,
    ...
    const maxSpeed = 60 / 3.6; // 60 km/h in m/s
    for(let i = 0; i <= 30; i++){
        const v = maxSpeed * i / 30;
        const f_rolling = mTotal * g_accel * cR;
        rollingResistance.push({
            velocity: v,
            force: f_rolling
        });
    }
    curves.push({
        name: "Rolling Resistance (0% grade)",
        points: rollingResistance,
        color: "#555555"
    });
    // Add grade resistance curves
    for(let gi = 1; gi < grades.length; gi++){
        ...
        curves.push({
            name: `Resistance (${(grade * 100).toFixed(0)}% grade)`,
            points: gradePoints,
...
//# sourceMappingURL=_c3b531d9._.js.map